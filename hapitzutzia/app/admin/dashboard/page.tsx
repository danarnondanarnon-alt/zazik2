...
interface Stats { totalRepairs: number; totalIncome: number; byStatus: Record<string, number>; }
interface RepairWithUnread extends Repair { unread_count?: number; }
...
export default function AdminDashboardPage() {
  ...
  useEffect(() => {
    ...
    loadData();
    // Show deleted toast if redirected after deletion
  ...
  const loadData = useCallback(async () => {
    ...
    const data: Repair[] = await fetch(`/api/repairs?${params}`).then(r => r.json());
    ...
    const withUnread = await Promise.all(
      repairList.map(async (r) => {
        try {
          const msgs = await fetch(`/api/repairs/${r.id}/messages`).then(res => res.json());
          const unread = Array.isArray(msgs)
            ? msgs.filter((m: { author_type: string; read_by_admin: boolean }) =>
                m.author_type === 'customer' && !m.read_by_admin).length
            : 0;
          return { ...r, unread_count: unread };
        } catch { return { ...r, unread_count: 0 }; }
      })
    );
  ...
  function logout() { clearAdminSession(); router.push('/admin'); }
  ...
  const newBtn = (
    <Link href="/admin/new-repair" className={styles.newBtn}>+ תיקון</Link>
  );
      ...
      <main className={styles.main}>
        ...
        {stats && (
          <div className={styles.statsGrid}>
            ...
            <div className={styles.statBox}>
              <div className={`${styles.statNum} ${styles.income}`}>₪{(stats.totalIncome ?? 0).toLocaleString()}</div>
              <div className={styles.statLabel}>הכנסה החודש</div>
            </div>
            <div className={styles.statBox}>
              <div className={`${styles.statNum} ${styles.blue}`}>{stats.byStatus?.working ?? 0}</div>
              <div className={styles.statLabel}>בעבודה</div>
            </div>
            <div className={styles.statBox}>
              <div className={`${styles.statNum} ${styles.green}`}>{stats.byStatus?.ready ?? 0}</div>
        ...
        {loading ? (
          ...
          <div className="stagger">
            {repairs.map(repair => {
              const daysInWork = repair.status === 'working' && repair.started_at
                ? Math.floor((Date.now() - new Date(repair.started_at).getTime()) / 86400000) : 0;
              const isAging = daysInWork > 14;
              return (
                <Link key={repair.id} href={`/admin/repairs/${repair.id}`}
                  className={`${styles.repairCard} ${isAging ? styles.aging : ''} animate-fade-up`}>
                    ...
                    <div className={styles.rcInfo}>
                      <div className={styles.rcTitle}>
                        ...
                        {(repair.unread_count ?? 0) > 0 && (
                          <span className={styles.unreadBadge}>💬 {repair.unread_count}</span>
                        )}
                      </div>
                      <div className={styles.rcSub}>
                        {repair.customer?.phone}
                        {isAging && <span className={styles.agingWarn}> · ⚠️ {daysInWork} ימים</span>}
                      </div>
                    </div>
                    <StatusBadge status={repair.status} />
                  </div>
                  <div className={styles.rcBottom}>
                    <span className={styles.rcDesc}>{repair.description.slice(0, 55)}{repair.description.length > 55 ? '…' : ''}</span>
                    {repair.price
                      ? <span className={styles.rcPrice}>₪{Number(repair.price).toLocaleString()}</span>
                      : <span className={styles.rcNoPrice}>טרם תומחר</span>}
                  </div>
                </Link>
              );
            })}
